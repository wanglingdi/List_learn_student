...
#define SECOND 2
//定义全局变量
int g_Passowrd=2170806;
//定义文本指针fp
FILE* fp;
//定义结构体包括学号 姓名 成绩
struct Student_Node//the struct of student (list);
{
    int data;
    char name[Max];
    float score;
...
//定义函数声明
//定义一个函数进行结点对结点的赋值；
struct Student_Node* GiveValue(struct Student_Node* SaveList,struct Student_Node* HeadList);
...
struct Student_Node analyse_score(struct Student_Node *head);// get Average and Max score from *head;
//定义一个函数，删除指定的ID的结点，以及删除两个链表相同部分
struct Student_Node* Delete_Score(struct Student_Node *head);//delete from File to *head;
//定义一个模式选择函数
void Mode_Choose(struct Student_Node *Head);
// This FUCT is to print a start;
void PrintInterFace(void);
//判断这是不是一个环链表
bool hasCycle(struct Student_Node *head);
...
struct Student_Node* toSortList(struct Student_Node* head, struct Student_Node* tail);
//定义归并排序函数的接口
...
{   
    ...
    struct Student_Node *DeleteList = (struct Student_Node*)malloc(sizeof(struct Student_Node));
    struct Student_Node *RDeleteList = (struct Student_Node*)malloc(sizeof(struct Student_Node));
        ...
        {   //计数器累加
            ...
            {   //检测到头节点是相同ID的结点
                ...
                {   //头指针指向第二个结点的地址
                    ...
                    //记录删除的链表到DeleteList中
                    //DeleteList=GiveValue(DeleteList,LastTempopary);
                    //DeleteList=DeleteList->next;
                    //释放掉头地址的空间
                    printf("the %d has been delete: %d  Name :%s  Score: %f \n",index,LastTempopary->data,LastTempopary->name,LastTempopary->score);
                    free(LastTempopary);
    ...
    OutputList(CmdTempopary);   
    //OutputList(DeleteList);
    }
    //进入两个链表删除相同元素模式
    if (SECOND ==choose)
    {   //初始化指针变量
                ...
                {   
                    ...
                    if (FastTempopary->data==NowTempopary->data)
                    {
                        CmdTempopary=NowTempopary->next;
                         printf("the %d has been delete: %d  Name :%s  Score: %f \n",index,NowTempopary->data,NowTempopary->name,NowTempopary->score);
...
{   //初始化局部变量密码
    ...
    printf("Error[00005]: your password is error\n");
    return false;
}
//定义一个模式选择函数
void Mode_Choose(struct Student_Node *Head)
...
struct Student_Node* GiveValue(struct Student_Node* SaveList,struct Student_Node* HeadList) {
    ...
    SaveList->data=HeadList->data;
    SaveList->name[Max]=HeadList->name[Max];
    SaveList->score=HeadList->score;
    return SaveList;
}
